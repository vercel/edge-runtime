...
The **@edge-runtime/ponyfill** helps to have the Edge Runtime APIs available in your environment:
...
<Tabs items={['npm', 'yarn', 'pnpm']} storageKey='selected-pkg-manager'>
  ...
  <Tab>
    ```sh yarn add @edge-runtime/ponyfill ```
    <Tab></Tab>
    ```sh pnpm install @edge-runtime/ponyfill --save ```
  </Tab>
</Tabs>
