...
The **@edge-runtime/format** receives any JavaScript primitive or Object as input, returning the beauty string represetantion as output.
...
<Tabs items={['npm', 'yarn', 'pnpm']} storageKey='selected-pkg-manager'>
  ...
  <Tab>
    ```sh yarn add @edge-runtime/format ```
    <Tab></Tab>
    ```sh pnpm install @edge-runtime/format --save ```
  </Tab>
</Tabs>
...
```js
format('Rhe PI number is', Math.PI, '(more or less)')
