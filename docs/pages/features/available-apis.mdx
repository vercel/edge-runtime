...
In addition to the following APIs, the runtime can be addressed by checking the `globalThis.EdgeRuntime` property. Using `if (typeof EdgeRuntime !== 'string') {...}` will enable dead-code elimination for the code inside the if block.
