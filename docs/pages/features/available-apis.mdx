...
In addition to the following APIs, you can address the runtime by checking the `globalThis.EdgeRuntime` property. Using `if (typeof EdgeRuntime !== 'string') {...}` will enable dead-code elimination for the code inside the if block.
